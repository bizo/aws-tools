  ...
  class CreateJobFlowCommand < StepProcessingCommand
    ...
    def role_exists?(role_name, client)
      begin
        ...
        if e.message =~ /NoSuchEntity/ || (e.message.include?("Instance Profile #{role_name} cannot be found"))
    ...
    def create_iam_client
      ...
      iam_endpoint = EmrConfig.iam_endpoint(@commands.global_options[:region], @commands.global_options[:suffix])
      iam_port = 443
      iam_region = @commands.global_options[:region]
    ...
    def create_role_and_profile(jobflow_role, client)
      ...
      client.create_role(:role_name => jobflow_role, 
                         :assume_role_policy_document => EC2Roles.get_role_def(EmrConfig.get_service_principal(@commands.global_options[:suffix])))
